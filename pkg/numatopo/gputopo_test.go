...
func TestReadNUMANode(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tmpDir, err := os.MkdirTemp("", "gputopo-test")
			if err != nil {
				t.Fatal(err)
			}
			...
			if err := os.WriteFile(numaFile, []byte(tt.content), 0644); err != nil {
				t.Fatal(err)
			}
...
func TestIsNvidiaGPU(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := os.WriteFile(filepath.Join(tmpDir, "vendor"), []byte(tt.vendor), 0644); err != nil {
				t.Fatal(err)
			}
			if err := os.WriteFile(filepath.Join(tmpDir, "class"), []byte(tt.class), 0644); err != nil {
				t.Fatal(err)
			}
...
func TestReadDeviceModel(t *testing.T) {
	...
	t.Run("with label", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(tmpDir, "label"), []byte("NVIDIA A100-SXM4-80GB"), 0644); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("without label", func(t *testing.T) {
		...
		if err := os.WriteFile(filepath.Join(tmpDir, "vendor"), []byte("0x10de"), 0644); err != nil {
			t.Fatal(err)
		}
		if err := os.WriteFile(filepath.Join(tmpDir, "device"), []byte("0x20b5"), 0644); err != nil {
			t.Fatal(err)
		}
...
func createFakePCIDevice(t *testing.T, baseDir, bdf, vendor, class, numaNode string) {
	...
	for name, content := range map[string]string{
		...
		if err := os.WriteFile(filepath.Join(devDir, name), []byte(content), 0644); err != nil {
			t.Fatal(err)
		}
	}
}
...
func TestDiscoverGPUs_FakeSysfs(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "fake-pci")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(tmpDir)
...
func TestDiscoverGPUs_EmptyDir(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "fake-pci-empty")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(tmpDir)
...
func TestDiscoverGPUs_NoNvidiaGPUs(t *testing.T) {
	tmpDir, err := os.MkdirTemp("", "fake-pci-no-gpu")
	if err != nil {
		t.Fatal(err)
	}
	defer os.RemoveAll(tmpDir)
...
func TestGetAllocatedGPUs_ValidCheckpoint(t *testing.T) {
	...
	tmpFile, err := os.CreateTemp("", "checkpoint")
	if err != nil {
		t.Fatal(err)
	}
